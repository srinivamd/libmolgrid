...
class ManagedGridBase {
  ...
  protected:
    ...
    void alloc_and_set_gpu(size_t sz) const {
      ...
      hipError_t err = hipMalloc(&gpu_info->gpu_ptr,sz*sizeof(Dtype));
      hipGetLastError();
      if(err != hipSuccess) {
    ...
    void clone_ptrs() {
      ...
      if(oldgpu.gpu_ptr && oldgpu.sent_to_gpu) {
        ...
        LMG_CUDA_CHECK(hipMemcpy(gpu_info->gpu_ptr, oldgpu.gpu_ptr, sizeof(Dtype)*capacity, hipMemcpyDeviceToDevice));
      }
    }
  public:
    ...
    size_t copyTo(cpu_grid_t& dest) const {
      ...
      if(sz == 0) return 0;
      if(ongpu()) {
        LMG_CUDA_CHECK(hipMemcpy(dest.data(), gpu_grid.data(), sz*sizeof(Dtype), hipMemcpyDeviceToHost));
      } else { //host ot host
    ...
    size_t copyTo(gpu_grid_t& dest) const {
      ...
      if(sz == 0) return 0;
      if(ongpu()) {
        LMG_CUDA_CHECK(hipMemcpy(dest.data(),gpu_grid.data(),sz*sizeof(Dtype),hipMemcpyDeviceToDevice));
      } else {
        LMG_CUDA_CHECK(hipMemcpy(dest.data(),cpu_grid.data(),sz*sizeof(Dtype),hipMemcpyHostToDevice));
      }
      return sz;
    }
    ...
    size_t copyFrom(const cpu_grid_t& src) {
      ...
      if(sz == 0) return 0;
      if(ongpu()) {
       LMG_CUDA_CHECK(hipMemcpy(gpu_grid.data(), src.data(), sz*sizeof(Dtype), hipMemcpyHostToDevice));
      } else {
    ...
    size_t copyFrom(const gpu_grid_t& src) {
      ...
      if(sz == 0) return 0;
      if(ongpu()) {
        LMG_CUDA_CHECK(hipMemcpy(gpu_grid.data(),src.data(),sz*sizeof(Dtype),hipMemcpyDeviceToDevice));
      } else {
        LMG_CUDA_CHECK(hipMemcpy(cpu_grid.data(),src.data(),sz*sizeof(Dtype),hipMemcpyDeviceToHost));
      }
      return sz;
    }
    ...
    size_t copyInto(size_t start, const ManagedGridBase<Dtype, NumDims>& src) {
      ...
      if(src.ongpu()) {
        if(ongpu()) {
          LMG_CUDA_CHECK(hipMemcpy(gpu_grid.data()+off,src.gpu_grid.data(),sz*sizeof(Dtype),hipMemcpyDeviceToDevice));
        } else {
          LMG_CUDA_CHECK(hipMemcpy(cpu_grid.data()+off,src.gpu_grid.data(),sz*sizeof(Dtype),hipMemcpyDeviceToHost));
        }
      } else { //on host
        if(ongpu()) {
         LMG_CUDA_CHECK(hipMemcpy(gpu_grid.data()+off, src.data(), sz*sizeof(Dtype), hipMemcpyHostToDevice));
        } else {
    ...
    void togpu(bool dotransfer=true) const {
      ...
      if(oncpu() && dotransfer) {
        LMG_CUDA_CHECK(hipMemcpy(gpu_info->gpu_ptr,cpu_ptr.get(),capacity*sizeof(Dtype),hipMemcpyHostToDevice));
      }
    ...
    void tocpu(bool dotransfer=true) const {
      if(ongpu() && capacity > 0 && dotransfer) {
        LMG_CUDA_CHECK(hipMemcpy(cpu_ptr.get(),gpu_info->gpu_ptr,capacity*sizeof(Dtype),hipMemcpyDeviceToHost));
      }
