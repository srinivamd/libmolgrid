...
inline hipMemcpyKind copyKind(bool srcCUDA, bool destCUDA) {
  if (srcCUDA) {
    if (destCUDA)
  ...
  public:
    ...
    */
    ...
    size_t copyTo(Grid<Dtype,NumDims,destCUDA>& dest) const {
      ...
      if(sz == 0) return 0;
      hipMemcpyKind kind = copyKind(isCUDA,destCUDA);
      LMG_CUDA_CHECK(hipMemcpy(dest.data(),data(),sz*sizeof(Dtype),kind));
      return sz;
    }
    ...
    size_t copyFrom(const Grid<Dtype,NumDims,srcCUDA>& src) {
      ...
      if(sz == 0) return 0;
      hipMemcpyKind kind = copyKind(srcCUDA,isCUDA);
      LMG_CUDA_CHECK(hipMemcpy(data(),src.data(),sz*sizeof(Dtype),kind));
      return sz;
    }
     ...
     */
    void fill_zero() {
      if(isCUDA) hipMemset(data(), 0, sizeof(Dtype)*size());
