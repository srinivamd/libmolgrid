...
template <typename Dtype>
 __host__ void Transform::forward(const Grid<Dtype, 2, true>& in, Grid<Dtype, 2, true>& out, bool dotranslate) const {
  ...
  if(dotranslate)
    hipLaunchKernelGGL(HIP_KERNEL_NAME(transform_forward_kernel<float, true>), dim3(LMG_GET_BLOCKS(N)), dim3(LMG_GET_THREADS(N)), 0, 0, N, Q, center, translate, in, out);
  else
    hipLaunchKernelGGL(HIP_KERNEL_NAME(transform_forward_kernel<float, false>), dim3(LMG_GET_BLOCKS(N)), dim3(LMG_GET_THREADS(N)), 0, 0, N, Q, center, translate, in, out);
}
...
template <typename Dtype>
 __host__  void Transform::backward(const Grid<Dtype, 2, true>& in, Grid<Dtype, 2, true>& out, bool dotranslate) const {
  ...
  if(dotranslate) {
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(transform_translate_kernel<Dtype>), dim3(LMG_GET_BLOCKS(N)), dim3(LMG_GET_THREADS(N)), 0, 0, N, untranslate, in, out);
    hipLaunchKernelGGL(HIP_KERNEL_NAME(transform_rotate_kernel<Dtype>), dim3(LMG_GET_BLOCKS(N)), dim3(LMG_GET_THREADS(N)), 0, 0, N,invQ,center,out,out);
  } else {
    hipLaunchKernelGGL(HIP_KERNEL_NAME(transform_rotate_kernel<Dtype>), dim3(LMG_GET_BLOCKS(N)), dim3(LMG_GET_THREADS(N)), 0, 0, N,invQ,center,in,out);
  }
