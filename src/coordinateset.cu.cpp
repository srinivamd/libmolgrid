...
void CoordinateSet::sum_types(Grid<float, 1, true>& sum, bool zerofirst) const {
  ...
  if(!has_vector_types()) {
    hipLaunchKernelGGL(sum_index_types_gpu, dim3(blocks), dim3(threads), 0, 0, type_index.gpu(), sum);
  } else { //vector types
    hipLaunchKernelGGL(sum_vector_types_gpu, dim3(blocks), dim3(threads), 0, 0, type_vector.gpu(), sum);
  }
